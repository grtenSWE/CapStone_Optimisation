...
if ischar(coord)  % Either a NACA string or a filename
  if ~isempty(regexpi(coord,'^NACA *[0-9]{4,5}$', 'once')) % NACA string supplied
    % NACA specified (e.g. 'NACA0012'); leave file_coord as default.
  else % Filename supplied
    % set coord file to the supplied filename
    file_coord = coord;
  end
else
  % Write foil ordinate file
  % Ensure coordinate file is written in the XFoil working directory
  file_coord_full = fullfile(wd, file_coord);
  if exist(file_coord_full,'file'),  delete(file_coord_full); end;
  fid = fopen(file_coord_full,'w');
  if (fid<=0)
...
if (fid<=0)
  ...
  if ischar(coord)
    if ~isempty(regexpi(coord,'^NACA *[0-9]{4,5}$', 'once'))  % NACA string supplied
      fprintf(fid,'naca %s\n',coord(5:end));
    else  % filename supplied (either from input, or written to disk)
      fprintf(fid,'load %s\n',file_coord_full);
    end 
